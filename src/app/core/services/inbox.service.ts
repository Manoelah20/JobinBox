...
const initialMessages: InboxMessage[] = [
    ...
    },
    {
        ...
        content:
            'Estão abertas as inscrições para o curso gratuito de React e TypeScript.',
        type: 'Curso',
        analyzed: false,
...
export class InboxService {
    ...
    private readonly _selectedMessageId =
        signal<string | null>(null);

    private readonly _extractedOpportunity =
        signal<ExtractedOpportunity | null>(null);

    readonly messages = this._messages.asReadonly();

    readonly selectedMessageId =
        this._selectedMessageId.asReadonly();

    readonly extractedOpportunity =
        this._extractedOpportunity.asReadonly();
    ...
    updateExtractedOpportunity(
        updates: Partial<ExtractedOpportunity>
    ): void {
        this._extractedOpportunity.update(
            (opportunity) =>
                opportunity
                    ? {
                        ...opportunity,
    ...
    private calculateRelevanceScore(
        ...
        // Nível Júnior
        if (
            /\bj[uú]nior\b|\bjr\b/i.test(content)
        ) {
            score += 8;
        }
    ...
    private extractType(
        ...
        if (
            /\bPJ\b|pessoa jurídica|pessoa juridica/i.test(
                content
            )
        ) {
            return 'PJ';
        ...
        if (
            /\bCLT\b|carteira assinada/i.test(content)
        ) {
            return 'CLT';
        }
    ...
    private extractWorkMode(content: string): string {
        if (
            /híbrid[ao]|hibrid[ao]|hybrid/i.test(content)
        ) {
            return 'Híbrida';
        }

        if (
            /presencial|on-site|onsite|office/i.test(content)
        ) {
            return 'Presencial';
        }

        if (
            /remota|remoto|home office|100%\s*remoto|100%\s*remote|remote/i.test(
                content
            )
        ) {
            return 'Remota';
    ...
    private extractLocation(content: string): string {
        ...
        if (
            /remota|remoto|home office|100%\s*remoto/i.test(
                content
            )
        ) {
            return 'Remoto';
